...
#ifndef INCLUDE_VDB_SPH_SYSTEM_DATA3_H_
#define INCLUDE_VDB_SPH_SYSTEM_DATA3_H_
...
public:
    ...
    vox::ConstArrayAccessor1<double> densities() const;
    
    //! Returns the density array accessor (mutable).
    vox::ArrayAccessor1<double> densities();
    
    //! Returns the pressure array accessor (immutable).
    vox::ConstArrayAccessor1<double> pressures() const;
    
    //! Returns the pressure array accessor (mutable).
    vox::ArrayAccessor1<double> pressures();
    ...
    double sumOfKernelNearby(const vox::Vector3D& position) const;
    ...
    //! used.
    //!
    double interpolate(const vox::Vector3D& origin,
    ...
    //! Returns the laplacian of the given values at i-th particle.
    vox::Vector3D laplacianAt(size_t i,
                         const vox::ConstArrayAccessor1<vox::Vector3D>& values) const;
...
private:
    ...
    double _targetDensity = vox::kWaterDensity;
...
#endif  // INCLUDE_VDB_SPH_SYSTEM_DATA3_H_
