...
void SphSolver3::setNegativePressureScale(
    ...
    _negativePressureScale = vox::clamp(newNegativePressureScale, 0.0, 1.0);
}
...
void SphSolver3::setSpeedOfSound(double newSpeedOfSound) {
    _speedOfSound = std::max(newSpeedOfSound, vox::kEpsilonD);
}
...
void SphSolver3::onBeginAdvanceTimeStep(double timeStepInSeconds) {
    ...
    LOG(INFO) << "Building neighbor lists and updating densities took "
...
void SphSolver3::onEndAdvanceTimeStep(double timeStepInSeconds) {
    ...
    LOG(INFO) << "Max density: " << maxDensity << " "
...
void SphSolver3::computePressure() {
    ...
    = targetDensity * vox::square(_speedOfSound) / _eosExponent;
    
    vox::parallelFor(
                     vox::kZeroSize,
                     numberOfParticles,
                     [&](size_t i) {
        p[i] = vox::computePressureFromEos(
                                           d[i],
                                           targetDensity,
                                           eosScale,
                                           eosExponent(),
...
void SphSolver3::accumulatePressureForce(
                                         const vox::ConstArrayAccessor1<vox::Vector3D>& positions,
                                         const vox::ConstArrayAccessor1<double>& densities,
                                         const vox::ConstArrayAccessor1<double>& pressures,
                                         vox::ArrayAccessor1<vox::Vector3D> pressureForces) {
    auto particles = sphSystemData();
    size_t numberOfParticles = particles->numberOfParticles();
    
    const double massSquared = vox::square(particles->mass());
    const vox::SphSpikyKernel3 kernel(particles->kernelRadius());
    ...
    vox::parallelFor(
        ...
        for (size_t j : neighbors) {
            ...
            if (dist > 0.0) {
                vox::Vector3D dir = (positions[j] - positions[i]) / dist;
...
void SphSolver3::computePseudoViscosity(double timeStepInSeconds) {
    ...
    vox::Array1<vox::Vector3D> smoothedVelocities(numberOfParticles);
    ...
    factor = vox::clamp(factor, 0.0, 1.0);
