...
#include "vdb_pci_sph_solver3.hpp"
...
void PciSphSolver3::accumulatePressureForce(
    ...
    vox::Array1<double> ds(numberOfParticles, 0.0);
    
    vox::SphStdKernel3 kernel(particles->kernelRadius());
    ...
    vox::parallelFor(
                     ...
                     [&] (size_t i) {
        p[i] = 0.0;
        _pressureForces[i] = vox::Vector3D();
    ...
    for (unsigned int k = 0; k < _maxNumberOfIterations; ++k) {
        ...
        _pressureForces.set(vox::Vector3D());
        ...
        for (size_t i = 0; i < numberOfParticles; ++i) {
            maxDensityError = vox::absmax(maxDensityError, _densityErrors[i]);
        }
    ...
    LOG(INFO) << "Number of PCI iterations: " << maxNumIter;
    LOG(INFO) << "Max density error after PCI iteration: " << maxDensityError;
    if (std::fabs(densityErrorRatio) > _maxDensityErrorRatio) {
        LOG(WARNING) << "Max density error ratio is greater than the threshold!";
        LOG(WARNING) << "Ratio: " << densityErrorRatio
...
double PciSphSolver3::computeDelta(double timeStepInSeconds) {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        if (distanceSquared < kernelRadius * kernelRadius) {
            ...
            vox::Vector3D direction =
            (distance > 0.0) ? point / distance : vox::Vector3D();
            
            // grad(Wij)
            vox::Vector3D gradWij = kernel.gradient(distance, direction);
            denom1 += gradWij;
...
double PciSphSolver3::computeBeta(double timeStepInSeconds) {
    auto particles = sphSystemData();
    return 2.0 * vox::square(particles->mass() * timeStepInSeconds
