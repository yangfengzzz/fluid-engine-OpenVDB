...
#include "vdb_point_particle_emitter3.hpp"
...
_speed(speed),
_spreadAngleInRadians(vox::degreesToRadians(spreadAngleInDegrees)) {
}
...
void PointParticleEmitter3::onUpdate(
    ...
    if (maxNumberOfNewParticles > 0) {
        vox::Array1<vox::Vector3D> candidatePositions;
        vox::Array1<vox::Vector3D> candidateVelocities;
...
void PointParticleEmitter3::emit(
    ...
    for (size_t i = 0; i < maxNewNumberOfParticles; ++i) {
        vox::Vector3D newDirection = uniformSampleCone(
                                                       random(),
                                                       random(),
                                                       _direction,
...
PointParticleEmitter3::Builder::withOrigin(const vox::Vector3D& origin) {
    _origin = origin;
    return *this;
}

PointParticleEmitter3::Builder&
PointParticleEmitter3::Builder::withDirection(const vox::Vector3D& direction) {
    _direction = direction;
    return *this;
}
