...
#include "vdb_particle_system_solver3.hpp"
...
ParticleSystemSolver3::ParticleSystemSolver3(
    ...
    _wind = std::make_shared<vox::ConstantVectorField3>(vox::Vector3D());
}
...
void ParticleSystemSolver3::setRestitutionCoefficient(
    ...
    _restitutionCoefficient = vox::clamp(newRestitutionCoefficient, 0.0, 1.0);
}

const vox::Vector3D& ParticleSystemSolver3::gravity() const {
    return _gravity;
}

void ParticleSystemSolver3::setGravity(const vox::Vector3D& newGravity) {
    _gravity = newGravity;
}
...
const vox::Collider3Ptr& ParticleSystemSolver3::collider() const {
    return _collider;
}
...
const vox::VectorField3Ptr& ParticleSystemSolver3::wind() const {
    return _wind;
}

void ParticleSystemSolver3::setWind(const vox::VectorField3Ptr& newWind) {
    _wind = newWind;
}
...
void ParticleSystemSolver3::onInitialize() {
    ...
    vox::Timer timer;
    updateCollider(0.0);
    LOG(INFO) << "Update collider took "
    << timer.durationInSeconds() << " seconds";
    
    timer.reset();
    updateEmitter(0.0);
    LOG(INFO) << "Update emitter took "
    << timer.durationInSeconds() << " seconds";
}
...
void ParticleSystemSolver3::onAdvanceTimeStep(double timeStepInSeconds) {
    ...
    LOG(INFO) << "Accumulating forces took "
    << timer.durationInSeconds() << " seconds";
    
    timer.reset();
    timeIntegration(timeStepInSeconds);
    LOG(INFO) << "Time integration took "
    << timer.durationInSeconds() << " seconds";
    
    timer.reset();
    resolveCollision();
    LOG(INFO) << "Resolving collision took "
    << timer.durationInSeconds() << " seconds";
...
void ParticleSystemSolver3::beginAdvanceTimeStep(double timeStepInSeconds) {
    ...
    setRange1(forces.size(), vox::Vector3D(), &forces);
...
void ParticleSystemSolver3::resolveCollision(
                                             vox::ArrayAccessor1<vox::Vector3D> newPositions,
                                             vox::ArrayAccessor1<vox::Vector3D> newVelocities) {
...
void ParticleSystemSolver3::accumulateExternalForces() {
    ...
    vox::parallelFor(
                     ...
                     [&] (size_t i) {
        // Gravity
        vox::Vector3D force = mass * _gravity;
        
        // Wind forces
        vox::Vector3D relativeVel = velocities[i] - _wind->sample(positions[i]);
...
void ParticleSystemSolver3::timeIntegration(double timeStepInSeconds) {
    ...
    vox::parallelFor(
        ...
        vox::Vector3D& newVelocity = _newVelocities[i];
        newVelocity = velocities[i]
        + timeStepInSeconds * forces[i] / mass;
        
        // Integrate position.
        vox::Vector3D& newPosition = _newPositions[i];
