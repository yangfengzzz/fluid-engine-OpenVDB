...
#ifndef vdb_particle_system_data3_hpp
#define vdb_particle_system_data3_hpp
...
public:
    //! Scalar data chunk.
    typedef vox::Array1<double> ScalarData;
    
    //! Vector data chunk.
    typedef vox::Array1<vox::Vector3D> VectorData;
    ...
    //!
    size_t addVectorData(const vox::Vector3D& initialVal = vox::Vector3D());
    ...
    vox::ConstArrayAccessor1<vox::Vector3D> positions() const;
    
    //! Returns the position array (mutable).
    vox::ArrayAccessor1<vox::Vector3D> positions();
    
    //! Returns the velocity array (immutable).
    vox::ConstArrayAccessor1<vox::Vector3D> velocities() const;
    
    //! Returns the velocity array (mutable).
    vox::ArrayAccessor1<vox::Vector3D> velocities();
    
    //! Returns the force array (immutable).
    vox::ConstArrayAccessor1<vox::Vector3D> forces() const;
    
    //! Returns the force array (mutable).
    vox::ArrayAccessor1<vox::Vector3D> forces();
    
    //! Returns custom scalar data layer at given index (immutable).
    vox::ConstArrayAccessor1<double> scalarDataAt(size_t idx) const;
    
    //! Returns custom scalar data layer at given index (mutable).
    vox::ArrayAccessor1<double> scalarDataAt(size_t idx);
    
    //! Returns custom vector data layer at given index (immutable).
    vox::ConstArrayAccessor1<vox::Vector3D> vectorDataAt(size_t idx) const;
    
    //! Returns custom vector data layer at given index (mutable).
    vox::ArrayAccessor1<vox::Vector3D> vectorDataAt(size_t idx);
    ...
    void addParticle(
                     ...
                     const vox::Vector3D& newVelocity = vox::Vector3D(),
                     const vox::Vector3D& newForce = vox::Vector3D());
    ...
    void addParticles(
                      ...
                      = vox::ConstArrayAccessor1<vox::Vector3D>(),
    ...
    //!
    const vox::PointNeighborSearcher3Ptr& neighborSearcher() const;
    ...
    void buildNeighborSearcher(double maxSearchRadius = 2.0);
    
    //! Builds neighbor lists with given search radius.
    void buildNeighborLists(double maxSearchRadius = 2.0);
...
private:
    ...
    vox::PointNeighborSearcher3Ptr _neighborSearcher;
...
#endif /* vdb_particle_system_data3_hpp */
