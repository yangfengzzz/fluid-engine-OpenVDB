...
vox::ConstArrayAccessor1<double> SphSystemData3::densities() const {
    return scalarDataAt(_densityIdx);
}

vox::ArrayAccessor1<double> SphSystemData3::densities() {
    return scalarDataAt(_densityIdx);
}

vox::ConstArrayAccessor1<double> SphSystemData3::pressures() const {
    return scalarDataAt(_pressureIdx);
}

vox::ArrayAccessor1<double> SphSystemData3::pressures() {
    return scalarDataAt(_pressureIdx);
}
...
double SphSystemData3::sumOfKernelNearby(const vox::Vector3D& origin) const {
    double sum = 0.0;
    vox::SphStdKernel3 kernel(_kernelRadius);
    neighborSearcher()->forEachNearbyPoint(
                                           origin, _kernelRadius,
                                           [&](size_t, const vox::Vector3D& neighborPosition) {
...
double SphSystemData3::interpolate(
                                   ...
                                   const vox::ConstArrayAccessor1<double>& values) const {
    double sum = 0.0;
    auto d = densities();
    vox::SphStdKernel3 kernel(_kernelRadius);
    const double m = mass();
...
vox::Vector3D SphSystemData3::interpolate(
                                          const vox::Vector3D& origin,
                                          const vox::ConstArrayAccessor1<vox::Vector3D>& values) const {
    vox::Vector3D sum;
    auto d = densities();
    vox::SphStdKernel3 kernel(_kernelRadius);
    const double m = mass();
...
vox::Vector3D SphSystemData3::gradientAt(
                                         size_t i, const vox::ConstArrayAccessor1<double>& values) const {
    vox::Vector3D sum;
    auto p = positions();
    auto d = densities();
    const auto& neighbors = neighborLists()[i];
    vox::Vector3D origin = p[i];
    vox::SphSpikyKernel3 kernel(_kernelRadius);
    const double m = mass();
    
    for (size_t j : neighbors) {
        vox::Vector3D neighborPosition = p[j];
        double dist = origin.distanceTo(neighborPosition);
        if (dist > 0.0) {
            vox::Vector3D dir = (neighborPosition - origin) / dist;
            sum += d[i] * m *
            (values[i] / vox::square(d[i]) + values[j] / vox::square(d[j])) *
...
vox::Vector3D SphSystemData3::laplacianAt(
                                          size_t i, const vox::ConstArrayAccessor1<vox::Vector3D>& values) const {
    vox::Vector3D sum;
    auto p = positions();
    auto d = densities();
    const auto& neighbors = neighborLists()[i];
...
void SphSystemData3::computeMass() {
    ...
    vox::BoundingBox3D sampleBound(
                                   vox::Vector3D(-1.5 * _kernelRadius, -1.5 * _kernelRadius,
                                                 -1.5 * _kernelRadius),
                                   vox::Vector3D(1.5 * _kernelRadius, 1.5 * _kernelRadius,
                                                 1.5 * _kernelRadius));
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        const vox::Vector3D& point = points[i];
        double sum = 0.0;
        
        for (size_t j = 0; j < points.size(); ++j) {
            const vox::Vector3D& neighborPoint = points[j];
