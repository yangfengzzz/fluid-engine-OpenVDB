...
JET_BEGIN_TEST_F(FlipSolver3, DamBreakingWithCollider) {
    ...
    for (vox::Frame frame; frame.index < 200; ++frame) {
        ...
        searcher = std::dynamic_pointer_cast<PointVDBSearcher3>(solver->particleSystemData()->neighborSearcher());
...
JET_BEGIN_TEST_F(FlipSolver3, RotatingTank) {
    ...
    collider->setOnBeginUpdateCallback([&](vox::Collider3* col, double t, double) {
        if (t < 1.0) {
            col->surface()->transform.setOrientation({{0, 0, 1}, 2.0 * t});
            static_cast<vox::RigidBodyCollider3*>(col)->angularVelocity = {0, 0, 2};
        } else {
            static_cast<vox::RigidBodyCollider3*>(col)->angularVelocity = {0, 0, 0};
        }
    });
